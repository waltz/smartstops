...
1) The module file should be deployed under the `modules.d/` (i.e. the current) directory. For example:
...
4) The module exectuable must accept a `run` subcommand. The module will be passed via STDIN a JSON object containing relevant information (see [Module Input](#module_input) below). Using this information, the module should print to STDOUT the response that will be send to the user. For example:
